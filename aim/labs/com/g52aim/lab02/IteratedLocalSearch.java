...
public class IteratedLocalSearch extends SinglePointSearchMethod {
    ...
    double sValue = problem.getObjectiveFunctionValue(CURRENT_SOLUTION_INDEX);
    problem.copySolution(CURRENT_SOLUTION_INDEX, BACKUP_SOLUTION_INDEX);

    for (int j = 0; j < intensityOfMutation; j++) {
    ...
    // Swap to original (backup) if new solution is unacceptable
    if (problem.getObjectiveFunctionValue(CURRENT_SOLUTION_INDEX)
        < sValue) {
      problem.copySolution(CURRENT_SOLUTION_INDEX, BACKUP_SOLUTION_INDEX);
    } else {
